...
Two containers from one image: `web` (dashboard/API on host port 5050 by
default — chosen specifically to avoid colliding with the more common
5000, which other projects on a shared server are more likely to already
be using; override with `WEB_PORT` in `.env`/passed to `deploy.sh` for a
different one. The container's own internal port always stays 5000, only
the `docker-compose.yml` port mapping's host side changes — served by
`waitress-serve --threads=8`, real concurrent request handling unlike
Flask's dev server) and
